...
export const StorageDropdown = () => {
  ...
  return (
    <div className="bg-white shadow-1 rounded-lg">
      ...
      >
        <p className="text-dark">Armazenamento</p>
        <button
          onClick={() => setToggleDropdown(!toggleDropdown)}
      ...
      >
        {STORAGES.map((storage) => (
          <label
            ...
            htmlFor={storage.id}
            className={
              ` cursor-pointer select-none flex items-center rounded-md ${storage.actived ? 'bg-blue text-white': ''}  hover:bg-blue hover:text-white`
            }
          >
            <div className="relative">
              <input type="radio" name="size" id={storage.id} className="sr-only" />
